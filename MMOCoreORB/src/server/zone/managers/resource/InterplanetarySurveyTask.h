...
public:
	...
	void run() {
		...
		ResourceSpawner *resSpawner = rmanager->getResourceSpawner();
		ResourceMap *resMap = resSpawner->getResourceMap();

		HashTable<String, Reference<Vector<String> *>> mapped;
		HashTable<String, Reference<Vector<String> *>> typeMap;

		auto zoneMap = resMap->getZoneResourceList(surveyData->getPlanet());
		if (zoneMap != nullptr)
		{
		...
		UnicodeString subject(String("Interplanetary Survey: " + planetName));
