...
public:
	...
	PlanetTravelPoint(const PlanetTravelPoint& ptp) : Object() {
		...
		pointCellID = ptp.pointCellID;
		pointParentID = ptp.pointParentID;
	...
	void readLuaObject(LuaObject* luaObject) {
		...
		pointParentID = luaObject->getIntField("parentid");
		pointCellID = luaObject->getIntField("cellid");
	}
	...
	inline const int& getPointCellID() const {
		return pointCellID;
	}

	inline const int& getPointParentID() const {
