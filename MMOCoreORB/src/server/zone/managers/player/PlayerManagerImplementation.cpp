...
int PlayerManagerImplementation::notifyDestruction(TangibleObject* destructor, TangibleObject* destructedObject, int condition, bool isCombatAction) {
	...
	if (!destructor->isPlayerCreature() || (destructor->isPlayerCreature() && playerCreature->getZone()->getZoneName() != "space_renegades_station"))
	{
...
void PlayerManagerImplementation::sendActivateCloneRequest(CreatureObject* player, int typeofdeath) {
	...
	if(zone->getZoneName() == "space_renegades_station" && player->hasSkill("force_rank_light_novice")){
		cloneMenu->addMenuItem("Jedi Enclave (Yavin4)", 8525417);
	}

	if(zone->getZoneName() == "space_renegades_station" && player->hasSkill("force_rank_dark_novice")){
		cloneMenu->addMenuItem("Sith Enclave (Yavin4)", 3435626);
	}
