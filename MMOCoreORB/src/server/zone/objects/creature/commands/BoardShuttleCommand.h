...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		bool ignoreShuttle = false; // for zones that shuttle's are available
		if (zone->getZoneName().contains("space_renegades"))
		{
			ignoreShuttle = true;
		}
		...
		if (shuttle == nullptr && !ignoreShuttle) {
			creature->error("WARNING: Missing a shuttle object for point [" + closestPoint->getPointName() + "]: Data: " + closestPoint->toString());
		...
		int parentID = arrivalPoint->getPointParentID();
		int cellID = arrivalPoint->getPointCellID();
		
		creature->switchZone(arrivalZone->getZoneName(), x, p.getPositionZ(), y, parentID + cellID);
