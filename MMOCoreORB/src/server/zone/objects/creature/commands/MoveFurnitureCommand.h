...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		try {
			...
			if (dir != "up" && dir != "down" && dir != "forward" && dir != "back" && dir != "x" && dir != "y")
				throw Exception("@player_structure:format_movefurniture_distance"); //Format: /moveFurniture <FORWARD/BACK/UP/DOWN> <distance>

			if (dir == "x" || dir == "y")
				cardinal = true;

			dist = tokenizer.getFloatToken();

			if ((dist < 0.001f || dist > 500.f) && !cardinal)
				throw Exception("The amount to move must be between 0.001 and 500"); //The amount to move must be between 1 and 500.

			if(cardinal && (dist < -500.001f || dist > 500.f))
				throw Exception("The amount to move must be between -500 and 500"); //The amount to move must be between 1 and 500.
